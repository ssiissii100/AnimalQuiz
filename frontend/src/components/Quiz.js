...
const Quiz = () => {
    ...
    const handleNextRound = () => {
        ...
        fetch(`http://localhost:3000/animals?category=${category}`)
    ...
    const handleCategorySelection = (selectedCategory) => {
        ...
        fetch(`http://localhost:3000/animals?category=${selectedCategory}`)
            .then(response => response.json())
            .then(data => {
                setAnimals(data);
                setCorrectAnimals(data[0]);
    ...
    return (
        <div>
            ...
            {category && <QuizPage animals={animals} handleAnimalsSelection={handleAnimalsSelection} />}
            {currentRound > 5 && <Result userScore={userScore} handleRestart={handleRestart} />}
        </div>
    );
};
