...
export default function Finish({timeSetter, time, name}) {
    ...
    //Post current User
    if (users != null) {
        if(name != "0" && time < 10000000 && time > 0){
            if(statusPost == false){
    ...
    return(
        <div className="finishContainer">
            ...
            <div className="bestResults">
                <h3>Best results:</h3>
                    {sortedUsers != null ? <>
                        <p>1. {sortedUsers[0].username}: {sortedUsers[0].points} Points in {sortedUsers[0].time} milliseconds</p>
                        <p>2. {sortedUsers[1].username}: {sortedUsers[1].points} Points in {sortedUsers[1].time} milliseconds</p>
                        <p>3. {sortedUsers[2].username}: {sortedUsers[2].points} Points in {sortedUsers[2].time} milliseconds</p>
                    </> : <></>}
            </div>
        </div>
    )
